...
import org.example.repository.RepositoryWithOptional;
...
public class CommunityComponents {
    private RepositoryWithOptional<Long, User> userRepository;
    ...
    public CommunityComponents(RepositoryWithOptional<Long, User> userRepository) {
...
//            }
//        }
        List<User> collect = StreamSupport.stream(userRepository.findAll().spliterator(), false).filter(user -> !visited[Math.toIntExact(user.getId()) - 1]).collect(Collectors.toList());
        ...
        while (!queue.isEmpty()) {
            ...
            if (current == theLostFriend) {
                ...
                while (z != null) {
                    ...
                    int indice = parent[Math.toIntExact(z.getId()) - 1];
                    Optional<User> x = userRepository.findOne(1L + indice);
                    if (x.isPresent()) {
                        z = x.get();
                    } else {
                        z = null;
